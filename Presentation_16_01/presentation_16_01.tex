\documentclass{beamer}

\usepackage[frenchb]{babel}
\usepackage[T1]{fontenc}
\usepackage[latin9]{inputenc}
\usepackage{graphicx}

\usetheme{CambridgeUS}




\AtBeginSection[]
{
  \begin{frame}
  \frametitle{Sommaire}
  \tableofcontents[currentsection]
  \end{frame} 
}


\title[1ere Soutenance Projet]{Soutenance mi-parcours}
\author{Equipe CFR}
\institute{ESILV}
\date{16 janvier}



\begin{document}

%%%%%%%%%%%%%%
%%% diapo 1 %%
%%%%%%%%%%%%%%

\begin{frame}
\titlepage
\begin{center}
\includegraphics[width=3cm]{img/Logo_ESILV.png}
\end{center}
\end{frame}
	

\section{Qui nous sommes ?}
\subsection{Notre équipe}


%%%%%%%%%%%%%%
%%% diapo 2 %%
%%%%%%%%%%%%%%


\begin{frame}
\frametitle{Une équipe de folie !}
\begin{block}{}
\begin{itemize}
\item 15 étudiants de l'école ESILV
\item Etudiants 2ème année / 3éme année 
\item Etudiants motivés 
\item Mentor en cours de thèse
\end{itemize}
\end{block}
\begin{center}
\end{center}
\end{frame}



\subsection{Notre association}


%%%%%%%%%%%%%%
%%% diapo 3 %%
%%%%%%%%%%%%%%


\begin{frame}
\frametitle{DaVinciBot, associtation de robotique}
\begin{center}
\includegraphics[height=3cm]{img/logo_assos.png}
\end{center}
\begin{block}{le but de l'association}
\begin{itemize}
\item Gérer les dépenses
\item Recevoir l'aide de sponsors
\end{itemize}
\end{block}
\end{frame}



\section{Coupe de France Robotique}
\subsection{Le déroulement}


%%%%%%%%%%%%%%
%%% diapo 4 %%
%%%%%%%%%%%%%%

\begin{frame}
\begin{columns}
\begin{column}{0.3\textwidth}
\includegraphics[width=3cm]{img/lune_pres.png}
\end{column}
\begin{column}{0.3\textwidth}
\includegraphics[width=3cm]{img/logo_coupe.jpg}
\end{column}
\begin{column}{0.3\textwidth}
\includegraphics[width=3cm]{img/photo_coupe02.jpg}
\end{column}
\end{columns}
\begin{columns}
\begin{column}{0.4\textwidth}
\begin{block}{La coupe !}
\begin{itemize}
\item 200 équipes
\item Système de battle 
\item 90 secondes 
\item thème : la lune 
\item 25 au 27 mai 2017
\item Roche-sur-Yon 
\end{itemize}
\end{block}
\end{column}
\begin{column}{0.4\textwidth}
\vspace{0.5cm}
\includegraphics[height=5cm]{img/maps.png}
\end{column}
\end{columns}
\end{frame}


\subsection{Les épreuves}


%%%%%%%%%%%%%%
%%% diapo 5 %%
%%%%%%%%%%%%%%

\begin{frame}
\frametitle{Notre mission cette année :}
\begin{block}{}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{itemize}
\item Ramasser les roches lunaires
\item Construire une base lunaire
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
\begin{itemize}
\item La funny action
\end{itemize}
\end{column}
\end{columns}
\end{block}
\begin{center}
\includegraphics[width=10cm]{img/Table_2017.png} 
\end{center}
\end{frame}



\section{Ingénieurs de demain... Déjà les outils en main !}


%%%%%%%%%%%%%%
%%% diapo 6 %%
%%%%%%%%%%%%%%

\begin{frame}
\frametitle{Nos outils :}
\begin{columns}
\begin{column}{0.4\textwidth}
\begin{block}{}
\begin{itemize}
\item Solidworks
\item \LaTeX - Mail 
\item GITHUB
\item ROS 
\item Raspberry - Arduino 
\end{itemize} 
\end{block}
\begin{center}
\includegraphics[width=3.5cm]{img/archi_1.png}
\end{center}
\end{column}
\begin{column}{0.5\textwidth}
\begin{center}
\includegraphics[width=3cm]{img/logo_latex.png}\\
\includegraphics[width=4cm]{img/logo_solidworks.jpg}\\
\includegraphics[width=4cm]{img/raspberry.jpeg}\\
\includegraphics[width=4cm]{img/logo_github.jpg}
\end{center}
\end{column}
\end{columns}
\end{frame}


\section{Répartitions des tâches}


%%%%%%%%%%%%%%
%%% diapo 7 %%
%%%%%%%%%%%%%%

\begin{frame}
\frametitle{Notre graphique des tâches :}
\begin{block}{}
Utilisation de graphviz
\end{block}
\begin{center}
\includegraphics[width=10cm]{img/graphviz.PNG} 
\end{center}
\end{frame}

\section{Avancement du projet}

%%%%%%%%%%%%%%
%%% diapo 8 %%
%%%%%%%%%%%%%%


\begin{frame}
\begin{columns}
\begin{column}{0.7\textwidth}
\begin{center}
	\begin{block}{}
			\begin{itemize}
				\item Réalisation de la table
				\item Prototype système de repérage en cours	
				\item Création du GIT
	 			\item Modélisation d'une base roulante + impression
				\item Préparation des raspberry Linux + ROS + Python
		 		\item Graphique de la structure de ROS
			\end{itemize}
	\end{block}
	\includegraphics[height=4cm]{img/table_maison.jpg}
	\end{center}
	\end{column}
	\begin{column}{0.3\textwidth}
			\begin{center}
				\includegraphics[height=5cm]{img/systeme_reperage.PNG} 	
			\end{center}
		\end{column}
	\end{columns}
	
	 
\end{frame}

\section{Questions}

%%%%%%%%%%%%%%
%%% diapo 9 %%
%%%%%%%%%%%%%%

\begin{frame}
\frametitle{Questions ?}
\begin{center}
L'équipe CFR vous remercie de votre écoute
\end{center}
\begin{alertblock}{\begin{center}Avez-vous des questions ?\end{center}}
\end{alertblock}

\begin{block}{}
\begin{center}Nous suivre\\
Sur notre site : \url{www.davincibot.org}  \\
Sur FaceBook : \url{www.facebook.com/DaVinciBotEsilv}\end{center}
\end{block}
\end{frame}


\end{document}